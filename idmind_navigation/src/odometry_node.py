        ...
        if self.simulation:
            ...
            try:
                ...
                if self.kinematics == "2wd":
                    ...
                    # Get state changes
                    self.wheels_lock.acquire()
                    front_left = self.wheel_odom["front_left"]
                    front_right = self.wheel_odom["front_right"]
                    back_left = self.wheel_odom["back_left"]
                    back_right = self.wheel_odom["back_right"]
                    self.wheels_lock.release()

                    dlinear_x = - front_left + front_right - back_left + back_right
                    dlinear_y = front_left + front_right - back_left - back_right

                    # If IMU is not usable, use wheel odometry readings
                    if not use_imu:
                        dth = (front_left + front_right + back_left + back_right) / self.base_width
                else:
                    dth = 0
                    dlinear_x = 0
                    dlinear_y = 0
